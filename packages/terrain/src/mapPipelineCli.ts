...
import { loadStyleProfile } from "./mapProfile.js";
import { runMapPipeline } from "./mapPipeline.js";

async function main(): Promise<void> {
  const mapId = process.argv[2];
  const styleId = process.argv[3] ?? "antique";
  if (!mapId) {
    throw new Error("usage: pnpm --filter @sengoku-jidai/terrain gen:map <mapId> [styleId]");
  }
  ...
  console.log(`[terrain] map pipeline for "${mapId}" (style: ${styleId}) → ${outDir}`);
